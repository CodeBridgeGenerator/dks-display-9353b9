...
const ProductsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,imgs: _entity?.imgs,description: _entity?.description,price: _entity?.price,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputText id="imgs" className="w-full mb-3 p-inputtext-sm" value={_entity?.imgs} onChange={(e) => setValByKey("imgs", e.target.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["imgs"]) ? (
              <p className="m-0" key="error-imgs">
                {error["imgs"]}
              </p>
            ) : null}
          </small>
